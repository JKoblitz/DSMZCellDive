	...
	(function ($, window, document, undefined) {
		...
		AmsifySuggestags.prototype = {
			...
			setEvents: function () {
				...
				if (window !== undefined) {
					$(window).resize(function () {
						// $(_self.selectors.listArea).width($(_self.selector).width() - 3);
					});
				}
				...
				$(this.selectors.sTagsInput).keyup(function (e) {
					...
					if (key == 'Enter') {
						...
						if (e.originalEvent.ctrlKey) {
							_self.controlKey = true
						} else {
							_self.controlKey = false
						}
						...
						var value = _self.getValue(value)
						var items = value.split(/[\s,;]+/);
						console.log(items);
						if (items.length) {
							$.each(items, function (index, item) {
								// console.log(item, _self.getValue(item));
								_self.addTag(_self.getValue(item));
							});
						}
						$(this).val('');
			...
			getTag: function (value) {
				if (this.settings.suggestions.length && !this.controlKey) {
					var tag = value;
			...
			processAjaxSuggestion: function (value, keycode) {
				...
				var params = { existingTags: this.tagNames, existing: this.settings.suggestions, term: value, type: this.settings.plotType };
				...
				ajaxFormParams['success'] = function (data) {
					...
					if (data && data.suggestions) {
						_self.settings.suggestions = data.suggestions //$.merge(_self.settings.suggestions, data.suggestions);
			...
			setDefault: function () {
				var _self = this;
				var items = $(this.selector).val().split(/[\s,;]+/);
				// console.log(items);
				if (items.length) {
					$.each(items, function (index, item) {
			...
			addTag: function (item, cl = "", manual = true, putative = false) {
				// console.log(item, cl);
				if (typeof item == "object") {
				...
				// console.log(itemKey);
				if (itemKey === -1 && cl == "" && manual) {
					cl = "bg-warning"

				}
				if ((!manual && putative) || (manual && this.altKey)) {
					cl = "bg-warning putative"
				}

				// changed: in our scenario, tags are you used for autocompletion. Values are displayed
				var html = '<span class="' + this.classes.tagItem.substring(1) + ' ' + cl + '" data-val="' + value + '">' + value + ' ' + this.setIcon() + '</span>';
				...
				if (this.settings.tagLimit != -1 && this.settings.tagLimit > 0 && this.tagNames.length >= this.settings.tagLimit) {
					...
					if (this.settings.tagLimit === 5){
						halfmoon.initStickyAlert({
							content: 'A maximum of 5 genes is supported for bar charts.',
							alertType: "alert-danger",
						})
					} else if (this.settings.tagLimit === 20){
						halfmoon.initStickyAlert({
							content: 'A maximum of 50 genes is supported for heat maps.',
							alertType: "alert-danger",
						})
					}
					return false;
				...
				if (this.isPresent(value)) {
					...
					if (tag != value) {
						dataVal = { value: dataVal, tag: tag }
					}
					this.tagNames.push(dataVal);
					this.setRemoveEvent();
					this.setInputValue();
					if (this.settings.afterAdd && typeof this.settings.afterAdd == "function" && manual) {
			...
			setInputValue: function () {
				var values = this.tagNames.map((d) => typeof d == "object" ? d.value + ":" + d.tag : d)
			...
			clear: function () {
				this._setMethod('clear');
				this.tagNames = [] // edit for emptying
				this._init();
			},
